...
(async () => {
  ...
  if (versionAfterFirstRun !== '4') throw new Error(`unexpected schema version after first run: ${versionAfterFirstRun}`);

  manager.ensureCompatibilityMigrations();
  const versionAfterSecondRun = db.prepare("SELECT value FROM system_settings WHERE key = 'schemaVersion'").get().value;
  if (versionAfterSecondRun !== '4') throw new Error(`unexpected schema version after second run: ${versionAfterSecondRun}`);
